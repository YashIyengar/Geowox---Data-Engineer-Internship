...
def tragedy_to_comedy():
    ...
    try:
        ...
        print("Number of Cases can't be string Value")
    except Exception as e:
        print(e)
    else:
        for i in range(cases):
            try:
                a,b = input("Please Enter 2 integers separated by a space:").split()  # This line of code accepts user input for two integers
                a = re.sub("^0+(?!$)", "", a)                                         # This regex pattern checks for any preceeding zeros for the inputed number
                b = re.sub("^0+(?!$)", "", b)
            except ValueError:
                print("Input numbers can't be string values")
            except Exception as e:
                print(e)
            else:
                try:
